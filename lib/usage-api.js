 ...
 *
 * Root cause of the "frozen gauge" bug:
...
/** Timeout for each individual HTTPS request to the usage API. */
...
// Force-refresh threshold: if cached data is older than this, attempt a fresh
// fetch even when an active backoff window would normally block it. After 6h of
// stale data the gauge is misleading enough that one retry attempt is worth it.
const FORCE_REFRESH_AGE_MS = 6 * 60 * 60 * 1000; // 6 hours
...
/**
 * Sentinel returned by fetchUsage() when no OAuth token is present.
 * Distinct from null (API error) so that getUsage() can avoid incrementing
 * failCount when the issue is simply a missing token, not a network/API error.
 */
...
async function getUsage() {
  ...
  // Bug C — Force-refresh after 6h: if the cached data is older than
  // FORCE_REFRESH_AGE_MS AND the backoff window has already elapsed (or was
  // never set), override the backoff and attempt a fresh fetch regardless of
  // failCount. Even if it fails again the gauge data is so stale that one
  // retry attempt is worth the risk.
  const cacheAge = disk ? now - disk.fetchedAt : Infinity;
  const forceRefresh = cacheAge >= FORCE_REFRESH_AGE_MS && now >= nextRetryAt;

  const fetchResult = (backoffActive && !forceRefresh) ? null : await fetchUsage();

  // NO_TOKEN means no OAuth credentials — not a transient API error.
  // Treat it as "skip" (no stale data advancement, no failCount increment).
  const noToken = fetchResult === NO_TOKEN;
  const data = (noToken || fetchResult === null) ? null : fetchResult;
  ...
  // Fetch failed (429, network, timeout, …) or was skipped due to an active
  // backoff window. Record/advance the backoff so the NEXT process (next
  // render) waits longer before retrying, instead of hammering the API again
  // immediately. Do NOT increment failCount for missing-token (noToken): that
  // is a configuration issue, not a transient API error, and retrying sooner
  // once the token is added should not be penalised.
  const skippedDueToBackoff = backoffActive && !forceRefresh;
  if (!skippedDueToBackoff && !noToken) {
